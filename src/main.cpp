...
// 示例回调函数 - 详细分析版本
void exampleEomCallback(SEMP_PARSE_STATE *parse, uint8_t messageType)
{
    printf("=== 消息解析成功 ===\n");
    printf("总长度: %d字节\n", parse->length);
    ...
    if (header->messageLength > 0) {
        ...
        if (header->messageLength == 2) {
            uint16_t value = (data[1] << 8) | data[0]; // 小端序
            printf("16位值 (小端): %d (0x%04X)\n", value, value);
            
            uint16_t valueBE = (data[0] << 8) | data[1]; // 大端序
    ...
    if (parse->length >= 4) {
        ...
        printf("CRC字节: 0x%02X 0x%02X 0x%02X 0x%02X\n", 
...
bool exampleBadCrcCallback(SEMP_PARSE_STATE *parse)
{
...
int main(void)
{
    ...
    for (size_t i = 0; i < sizeof(testData); i++) {
        sempProcessByte(&parser, testData[i]);
    }
